...
\begin{minipage}{0.6\textwidth}
	\centering
	\includegraphics[width=\textwidth]{memorymodel.png}
\end{minipage} \hfill
\begin{minipage}{0.4\textwidth}
...
Radeon Rays exposes a well defined C++ API for scene construction and performing asynchronous ray intersection queries. Radeon-Rays employs OpenCL kernels to do all these calculations and to speed up the ray intersection they order their objects in bounding volume hierarchies. Seeing they use OpenCL to speed up their computations by sending it of to the GPU, this library will run on almost any GPU (meaning it will also work on Nvidia/Intel products).
...
\begin{itemize}
	...
	\item{GLM}\newline\par
	When it comes to math libraries that plug in nicely with an OpenGL renderer there's really only one good option namely GLM. OpenGL Mathematics (GLM) is a header only C++ mathematics library for graphics software based on the OpenGL Shading Language (GLSL). GLM exposes a well defined interface for everything from general calculus to quaternion rotations (e.g. vector operations, matrix calculations).
\end{itemize}
...
The third rendering pipeline being forward+ although the most performant will not be covered with explicit detail. The reason that this technique is not that applicable has primarily to do with the fact that we have no way of postponing lighting effects. This technique also covers lighting object per object and that is precisely the opposite of what raytracing is trying to overcome.\newline\par
Forward+ is a rendering technique that improves upon regular forward rendering. Forward+ first determines which lights are overlapping which ares of the screen (screen space). During the shading phase that list is then consulted to only account for the lights that are potentially overlapping the current pixel fragment. The forward+ technique consists primarily of three simple passes.\newline\par

\pagebreak

\subsubsection{Ray construction using g-buffers}
...
Assimp as the name implies is an asset import library that handles about every relevant model export type that exist. The assimp api needs to be built using CMake and can then be linked into our solution. Then it's just a simple case of creating an instance of their importer context and calling the readfile method.
